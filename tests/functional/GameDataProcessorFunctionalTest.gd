...
func test_go_downstairs_view_map_talk_elodie_restart():
    ...
    var talk_render = _processor.process_action(InstructionSet.TALK, "elodie")
...
func test_go_downstairs_talk_elodie_auto_get_dagger_check_inventory():
    _restart_and_continue()
    _processor.process_action(InstructionSet.GO, "downstairs")

    var talk_auto_take_render = _processor.process_command_text("talk to elodie")
    ...
    # var inventory_render = _processor.process_command_text("i")
