...
func test_map_shows_map_name():
    var instruction = MapInstruction.new(_mock_areas, _mock_areas["frita batidos"])
    var map = instruction.execute()
    assert_string_contains(map, "ANN ARBOR")

func test_map_of_ann_arbor():
    var instruction = MapInstruction.new(_mock_areas, _mock_areas["library"])
    var map = instruction.execute()
...
func test_map_of_lansing():
    var instruction = MapInstruction.new(_mock_areas, _mock_areas["frandor"])
    var map = instruction.execute()
